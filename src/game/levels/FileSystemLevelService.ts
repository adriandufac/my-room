...
export class FileSystemLevelService {
  ...
  static async saveLevel(levelData: LevelData): Promise<boolean> {
    try {
      ...
      console.log(
        `[SUCCESS] Niveau "${levelData.name}" sauvegardé sous: ${fileName}`
      );
      return true;
    } catch (error) {
  ...
  static async getAllLevels(): Promise<Record<string, LevelData>> {
    try {
      ...
      try {
        ...
        console.log(
          `[FILE] ${
            Object.keys(customLevels).length
          } niveaux chargés depuis Data/levels/custom/`
        );
      } catch (error) {
        console.log(
          "[FILE] Aucun niveau personnalisé trouvé dans src/Data/levels/custom/"
        );
      }
  ...
  private static generateFileInstructions(
    levelData: LevelData,
    fileName: string
  ): void {
  ...
  static generateFileForManualSave(levelData: LevelData): string {
    ...
    console.log(
      `[COPY] Copier ce contenu dans src/Data/levels/custom/${fileName}:`
    );
    console.log(content);
