...
export default class DeskLamp extends FurnitureItem {
  ...
  constructor() {
    const config: FurnitureConfig = {
      ...
      clickable: true,
      hoverable: true,
      clickType: "desk-lamp", // Identifiant pour les événements
    };

    super(config);

    this.listenToEvent("uiLampToggle", this.onUIToggle.bind(this));
  }
  ...
  protected onModelSetup(model: THREE.Group): void {
    ...
    this.light.position.set(-0.4, 0.2, 0); // Position relative au modèle
    this.light.castShadow = true;
    /* this.light.target.position.set(0, -1, 0);
    model.add(this.light.target); */

    model.add(this.light);

    this.lightHelper = new THREE.SpotLightHelper(this.light);
    this.lightHelper.visible = false; // Par défaut, l'aide est visible
    model.add(this.lightHelper); // Ajouter à la scène, pas au modèle
  }
  ...
  public toggleLight(): void {
    ...
    // ✅ Émettre l'événement (méthode héritée)
    this.emitEvent("lampToggle", {
      lampId: this.config.clickType,
      isOn: this.isLightOn,
      intensity: this.isLightOn ? 10 : 0,
    });
  }

  private onUIToggle(event: CustomEvent): void {
    const { lampId, isOn } = event.detail;
    if (lampId === this.config.clickType) {
      this.setLightState(isOn);
    }
  }

  public setLightState(isOn: boolean): void {
    this.isLightOn = isOn;
    if (this.light) {
      this.light.intensity = isOn ? 1 : 0;
      if (this.lightHelper) this.lightHelper.update();
    }
  }
